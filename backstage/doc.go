...
OPTIONS
	...
	--keyless,		-k		Allow keyless use
							Indicate if it's allowed to make requests without authentication.

   	--subdomain,	-s 		Desired subdomain
							The subdomain will be used by the proxy: http://ratings.backstageserver.org. (where `ratings` is the chosen subdomain).

   	--team,			-t 		Team responsible for the service
   	--timeout 				Timeout
							Default: 0 (Do nothing. Wait the api server to return timeout.)
							If the API calls fail to respond after the timeout, the gateway will return an error.
